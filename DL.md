...
## 2026-05-12 — Delivery для `rust-v0.130.0`: без PR, прямой merge в main
**Решение:**
- Не использовать PR как delivery path для текущего upstream update.
- После green test gate смержить `chore/upstream-rust-v0.130.0` напрямую в `main` и запушить `main`.
- Draft PR #1 оставить как исторический артефакт предыдущего шага, если не потребуется отдельное закрытие.

**Причина:**
- Пользователь явно изменил процесс: "PR не делай, сразу мерж в main".
- Ветка уже содержит крупный upstream merge и targeted fixes; дальнейшая задача — довести локальный gate до green и доставить напрямую.

**Подтверждение:**
- `NOW.md` обновлён новым delivery policy.
- Merge в `main` пока не выполнен, потому что full workspace test не green и был прерван по просьбе пользователя.
